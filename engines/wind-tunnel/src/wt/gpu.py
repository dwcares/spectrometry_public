...
Same contract as ``Oscilloscope\\osc\\gpu.py`` (my standing policy since 2026-07-18: the GPU is the
...
cupy and rely on the pip-installed ``cupy-cuda12x[ctk]`` wheels. The ``[ctk]`` wheel bundles its
own CUDA 12 pieces, so no system CUDA install or CUDA_PATH change is needed.
